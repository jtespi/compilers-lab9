...
/* check the formal parameter list to see if they match length and types */
/*  Returns 1 if they match, 0 if they do not */
int CompareFormals( ASTnode * pfparams,  ASTnode * parglist ) {
      if ( (pfparams == NULL) && (parglist == NULL) )
        return (1);
	
    
    if ( (pfparams == NULL) || (parglist == NULL) ){
         printf(" Compare formals: length mismatch\n");
         return (0);
      }
    

    if ( pfparams -> isType == parglist -> isType ) {
        return (CompareFormals ( pfparams -> next, parglist -> next ));
      }
